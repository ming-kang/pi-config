...
- **Status transitions are gated.** `completed` can be reopened to `in_progress` or `pending`, but a `deleted` tombstone is terminal. Reopening lets a premature completion recover without losing the task id and its `blockedBy` edges. Single-active is enforced on demote (not hard-reject) so the model does not need a retry loop.
...
- **Overlay paint is pure + width-cached.** Completion visibility bookkeeping runs in `update()` plus a disposable timeout, never during `render`. Same terminal width reuses the last line array; state changes, expiry, and theme invalidation rebuild it.
- The overlay body is capped at 10 rows and prioritizes `in_progress`, ready pending, blocked pending, then recent completions; its truncation summary includes hidden status counts.
