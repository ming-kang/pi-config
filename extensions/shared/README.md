...
| `render-cache.ts` | Width-keyed render cache for custom TUI components. | Dialogs/selectors that wrap or truncate lines and must stay correct after resize. |
| `search-selector.ts` | Generic searchable selector component. | Menus such as `/advisor` that choose from many labeled options. |
| `dialog-primitives.ts` | Low-level dialog render helpers: rule borders, wrapped prefix text, focused rows. | Custom dialogs that should share the same selection/border behavior. |
