...
- **Write refresh skips a second disk read.** After a successful `write`, the hash is taken from the utf-8 `content` argument (same encoding Pi's write tool uses on disk); only mtime needs a `stat`. `read` / `edit` still hash on-disk bytes (result text can be partial or line-numbered; edit args are a patch, not final content).
